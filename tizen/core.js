...
(function (root, factory) {
   ...
   * item carries its own timestamp and is merged on its own. A film marked on
   * the television and a film marked on the phone both survive, and unmarking
   * on one device beats an older mark on the other, because "off" is a value
   * with a time on it and not the absence of one.
   *
   * doc: { v, history: [entry], favorites: {id: {at, on}}, watched: {id: {at, on}} }
   */
  const SYNC_VERSION = 1;
  const SYNC_HISTORY_CAP = 60;
  const SYNC_FLAG_CAP = 4000;

  function emptyDoc() {
    return { v: SYNC_VERSION, history: [], favorites: {}, watched: {} };
  }

  /** Ids that are switched on right now, newest decision first. */
  function flagsOn(map) {
    const on = [];
    Object.keys(map || {}).forEach(function (id) {
      const cell = map[id];
      if (cell && cell.on) on.push({ id: id, at: cell.at || 0 });
    });
    on.sort(function (a, b) {
      const d = (b.at || 0) - (a.at || 0);
      return d !== 0 ? d : (a.id < b.id ? -1 : (a.id > b.id ? 1 : 0));
    });
    return on.map(function (x) { return x.id; });
  }

  /** Turn a plain list of ids into timestamped flags, for a first upgrade. */
  function flagsFrom(ids, at) {
    const map = {};
    (ids || []).forEach(function (id, index) {
      // Older entries sit further back in time, so a later toggle anywhere wins.
      if (id) map[id] = { at: (at || 0) - (ids.length - index), on: true };
    });
    return map;
  }
  ...
  function mergeFlags(a, b) {
    ...
    const take = function (map) {
      Object.keys(map || {}).forEach(function (id) {
        ...
        if (!seen || at > seen.at || (at === seen.at && cell.on)) {
          out[id] = { at: at, on: !!cell.on };
        }
      });
    };
    take(a);
    ...
    const ids = Object.keys(out).sort(function (x, y) {
      const d = (out[y].at || 0) - (out[x].at || 0);
      return d !== 0 ? d : (x < y ? -1 : (x > y ? 1 : 0));
    });
    const capped = {};
    ids.slice(0, SYNC_FLAG_CAP).forEach(function (id) { capped[id] = out[id]; });
    return capped;
  }
  ...
  function mergeHistories(a, b) {
    ...
    const take = function (list) {
      (list || []).forEach(function (entry) {
        ...
        const seen = byId[entry.id];
        if (!seen || (entry.at || 0) > (seen.at || 0)) byId[entry.id] = entry;
      });
    };
    take(a);
    take(b);
    ...
    return Object.keys(byId)
      ...
      .sort(function (x, y) {
        const d = (y.at || 0) - (x.at || 0);
        return d !== 0 ? d : (x.id < y.id ? -1 : (x.id > y.id ? 1 : 0));
      })
      .slice(0, SYNC_HISTORY_CAP);
  }
   ...
   * reload — and that snapshot carries the portal's own address, username and
   * password, because that is what it takes to play the thing again. None of
   * that belongs on a sync server. The fields are listed rather than excluded:
   * a whitelist that forgets a field loses a poster, a blacklist that forgets
   * one leaks a password.
   */
  const WIRE_CARD_FIELDS = ['id', 'name', 'group', 'kind', 'contentType', 'logo'];
  const WIRE_ENTRY_FIELDS = ['id', 'at', 'position', 'duration'];

  function sanitizeDoc(doc) {
    const src = doc || emptyDoc();
    const history = (src.history || []).map(function (entry) {
      const out = {};
      WIRE_ENTRY_FIELDS.forEach(function (key) {
        if (entry[key] !== undefined) out[key] = entry[key];
      });
      if (entry.card) {
        const card = {};
        WIRE_CARD_FIELDS.forEach(function (key) {
          if (entry.card[key] !== undefined && entry.card[key] !== null) {
            card[key] = entry.card[key];
          }
        });
        out.card = card;
      }
  ...
  function mergeDocs(local, remote) {
    const a = local || emptyDoc();
    const b = remote || emptyDoc();
    return {
      v: SYNC_VERSION,
      history: mergeHistories(a.history, b.history),
      favorites: mergeFlags(a.favorites, b.favorites),
      watched: mergeFlags(a.watched, b.watched)
    };
  }
